...
import { ForceUpdateStoreSeerPaymentMethodCommand } from './force-update-store-payment-method.command';
...
@CommandHandler(ForceUpdateStoreSeerPaymentMethodCommand)
export class ForceUpdateStoreSeerPaymentMethodHandler
  implements ICommandHandler<ForceUpdateStoreSeerPaymentMethodCommand>
{
  constructor(
  ...
  async execute(command: ForceUpdateStoreSeerPaymentMethodCommand) {
    ...
    try {
      ...
      storeAggregate.forceUpdateStoreSeerPaymentMethod(
        correlationId,
        { ...payment_method },
        payment_method_id,
      );
