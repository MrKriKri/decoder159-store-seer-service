...
@Injectable()
export class StoreSeerRepository {
  constructor(
  ...
  async isStoreSeerExistForOwner(ownerId: string): Promise<boolean> {
  ...
  async get(id: string) {
    ...
    const storeSeerAggregate = new StoreSeerAggregate();
  ...
  async save(storeSeerAggregate: StoreSeerAggregate, correlationId: string) {
    try {
