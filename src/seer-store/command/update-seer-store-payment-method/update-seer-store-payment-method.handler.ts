...
import { UpdateSeerStorePaymentMethodCommand } from './update-seer-store-payment-method.command';
...
@CommandHandler(UpdateSeerStorePaymentMethodCommand)
export class UpdateSeerStorePaymentMethodHandler
  implements ICommandHandler<UpdateSeerStorePaymentMethodCommand>
{
  constructor(
  ...
  async execute(command: UpdateSeerStorePaymentMethodCommand) {
    ...
    try {
      ...
      storeAggregate.updateSeerStorePaymentMethod(
        correlationId,
        { ...payment_method },
        payment_method_id,
      );
